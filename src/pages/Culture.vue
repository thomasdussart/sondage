...
export default {
  data() {
    return {
      questions: [
        {
          options: [
            "Créer un mini-festival de musique de rue avec des jeunes artistes locaux",
            "Lancer un atelier de création de fresques murales dans le centre-ville",
          ],
        },
        {
          options: [
            "Organiser une soirée cinéma de plein air avec des films choisis par les jeunes",
            "Mettre en place une scène ouverte lors de la fin des examens pour des performances artistiques locales",
          ],
        },
        {
          options: [
  ...
  methods: {
    handleOptionClick(index) {
      if (this.currentQuestionIndex < this.questions.length - 1) {
        if (index === 0) {
          ...
          axios.post("http://localhost:1337/questions-culture", {
    ...
    markCategoryComplete() {
      ...
      if (!completedCategories.includes("Culture")) {
        completedCategories.push("Culture");
      }
